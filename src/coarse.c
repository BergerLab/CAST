...
void cb_coarse_get_all_links(struct cb_coarse_db_read *coarse_db){
    ...
    num_link_vectors = ftell(coarse_db->coarsedb->file_links_index)/8;
...
void cb_coarse_get_all_residues(struct cb_coarse_db_read *coarse_db){
    ...
    fseek(coarse_db->coarsedb->file_fasta_index, 0, SEEK_SET);
    ...
    fseek(coarse_db->coarsedb->file_fasta_base_index, 0, SEEK_SET);
...
cb_coarse_read_init(int32_t seed_size,
    ...
    coarsedb->coarsedb = cb_coarse_init(seed_size, file_fasta, file_seeds,
                                        ...
                                        file_links_base_index,
                                        file_links_count_index,
    ...
    coarsedb->seq_base_indices =
      malloc(coarsedb->num_coarse_seqs*sizeof(*(coarsedb->seq_base_indices)));
...
void cb_coarse_db_read_init_blocks(struct cb_coarse_db_read *coarse_db,
    ...
    seq_link_counts=malloc(coarse_db->num_coarse_seqs*sizeof(*seq_link_counts));
    assert(seq_link_counts);

    link_count = 0;

    /*Read the base index for each coarse FASTA sequence*/
    for (i = 0; i < coarse_db->num_coarse_seqs; i++) {
